...
void CLightOrigin::Spawn()
{
	if (angFEnv)
	{
		...
		if (pEntity)
		{
			...
			DevMsg("light_environment found\n");
		}
	}
}
...
CEnvCascadeLightSecond::CEnvCascadeLightSecond(void)
{
	...
	Q_strcpy(m_SpotlightTextureName.GetForModify(), "tools\\cascade\\mask_ring");
...
CEnvCascadeLight::CEnvCascadeLight(void)
{
	...
	Q_strcpy(m_SpotlightTextureName.GetForModify(), "tools\\cascade\\mask_center");
	m_nSpotlightTextureFrame = 0;
	m_LinearFloatLightColor.Init(1.0f, 1.0f, 1.0f);
	m_flAmbient = 0.0f;
	m_flNearZ = 8000.0f;
