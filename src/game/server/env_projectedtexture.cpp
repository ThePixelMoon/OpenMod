...
private:
	...
	// 0 = Non-OMOD projected texture, e.g. one that uses the VDC parenting fix instead of the spawnflag
...
{
#ifdef OMOD
	// env_projectedtexture's "Target" uses FIELD_EHANDLE while OMOD's base entity "SetTarget" uses FIELD_STRING
	if (inputdata.value.FieldType() != FIELD_EHANDLE)
		inputdata.value.Convert(FIELD_EHANDLE);
#endif
...
{
#ifndef OMOD // Putting this in Activate() breaks projected textures which start off or don't start always updating in savegames. Moved to Spawn() instead
	m_bState = ( ( GetSpawnFlags() & ENV_PROJECTEDTEXTURE_STARTON ) != 0 );
	m_bAlwaysUpdate = ( ( GetSpawnFlags() & ENV_PROJECTEDTEXTURE_ALWAYSUPDATE ) != 0 );
#endif
	...
	{
		// Pre-OMOD projected textures should use the VDC parenting fix.
		m_bAlwaysUpdate = true;
	}
#endif
